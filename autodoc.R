...
process <- function() {
  re <- "\\.yml$"
  files <- dir("man-roxygen", pattern=re)
  for (f in files) {
    base <- sub(re, "", f)
    cl <- get(paste0(".R6_", base))
    dat <- add_usage(yaml_read(file.path("man-roxygen", f)), cl)
    str <- format_class(dat)
    dest <- sprintf("man-roxygen/%s_methods.R", base)
